<?php
    ...
    include '../classes/Quotation.php';
    include '../function/function.php';

    /************************************ DECLARATION *********************************/
    $objQuotes = new Quotation();

    $list_quotes = $objQuotes->getQuote();

    upload_quotation_file($_POST['update'], $_POST['txtamount'], $_FILES['file_quote'], $_POST['quote_id']);

    $table_quotes = "";

    /************************************ OPERATION *********************************/
    foreach ($list_quotes as $value) {
        $invoice_link = $value['upload'];
        $invoice_link = isset($invoice_link) && $invoice_link != "NULL" ? "<a href='../uploads/".$invoice_link."'>View invoice</a>" : "N/A";

        $status = " <a href='quotation.php?id={$value['id']}' style='color: #1e7e34' title='Process' class='fa fa-cog'></a> &nbsp;";
  
        if($value['status'] == "Processed")
        {
            $status = "<a href='#' name='addfile' data-toggle='modal' data-target='#smallmodal' style='color: #0069d9' title='Add file and amount' class='fa fa-plus'></a>";
        } 

        $table_quotes .= "
            <tr>
                <td id='fullname'>".$value['fullname']."</td>
                ...
                <td>".$value['payment']."</td>
                <td>".$value['status']."</td>
                <td id='del' width='50px'>
                    <a href='#' name=".$value['id']." title='Delete' class='fa fa-trash-o' style='color: #dc3545;'></a> &nbsp;/
                    {$status}
                </td>
            </tr>";
...
<body>
    ...
    <script type="text/javascript">
        ...
        // $('#del a:nth-child(2)').click(function()
        // {
        ...
        // assign delete icon value to hidden input
        $(".content").find('a[name=addfile]').each(function()
        {
            $(this).click(function()
            {
                var quote_id = $(this).prev().attr('name')
