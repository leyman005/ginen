<?php 
   ...
   /************************************ DECLARATION *********************************/
    // If user logged in from different browser, it kill the session and keep the lates connction
    $user = new User();
    ...
    $sql = "
        SELECT
            at.ticket_ref, t.title
        FROM 
            assign_ticket at
        LEFT JOIN 
            tickets t ON (at.ticket_ref = t.ticket_id)
        LEFT JOIN
            users u ON (u.user_id = at.user_ref)
        WHERE u.user_id IN ({$user_id}) AND t.status = '1'
    ";

    $result = $user->connection->query($sql);

    while($myTickets = $result->fetch_object())
    {
        $ticket_id = $myTickets->ticket_ref;
        $title = $myTickets->title;

        if($_SESSION["user_info"]["manage"] == 'super user')
        {
            $control_buttons = "<a href='#' id='del' name='".$ticket_id."' class='fa fa-times' title='delete ticket' style=' margin-left: 7px; color: red'></a>";
            $control_buttons .= "<a href='#' class='fa fa-pencil' title='edit ticket' style=' margin-left: 7px; color: #8AB4F8'></a>";
        }

        $lists .= "
            <li>
                <label>
                    <input type='checkbox'><i class='check-box'></i><span>{$title}</span>
                    <a href='viewticket.php?id=$ticket_id' class='fa fa-play' title='view ticket' style='color: #1e7e34; cursor:pointer; margin-left: 10px'></a>
                    ".$control_buttons."
            ...
            <div id="main-menu" class="main-menu collapse navbar-collapse">
                <ul class="nav navbar-nav">
                  ...
                  <!--   <li>
                    ...
                    <li class="menu-title">Tickets</li><!-- /.menu-title -->

                    <li class="menu-item-has-children dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">  <i class="menu-icon fa fa-tasks" ></i>Tickets</a>
                         <ul class="sub-menu children dropdown-menu">
                             <li><i class="menu-icon fa fa-fort-awesome"></i><a href="newticket.php">New ticket</a></li>
                             <li><i class="menu-icon fa fa-fort-awesome"></i><a href="listticket.php">List ticket</a></li>
                        </ul>
                    </li>
                    <li class="menu-item-has-children dropdown">
